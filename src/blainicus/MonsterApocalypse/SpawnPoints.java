...
public class SpawnPoints implements Runnable, CommandExecutor {
    ...
    String name, worldnamez, propertyname, spawnnames;
    String returnstring;
    EntityType type;
    int period, count, min, max;
    double chance, x, y, z;
    ...
    public SpawnPoints(MonsterApocalypse instance) {
        ...
        plugin = instance;
        getspawnlist();
        pointlist = new ArrayList<spawnpoint>();
        fillpointlist();
        setconfig();
        save();
    }
    ...
    private String getproperty(String[] args) {
        try {
            ...
            if (propertyname.equalsIgnoreCase("type")) {
                ...
                return propertyname + ": " + selectedpoint.getPos().getWorld().getName() + ", " + selectedpoint.getPos().getX() + ", "
                       + selectedpoint.getPos().getY() + ", " + selectedpoint.getPos().getZ() + ".";
            } else if (propertyname.equalsIgnoreCase("count")) {
    ...
    private String getlist() {
        spawnnames = "Spawn Points: ";
        for (int i = 0; i < pointlist.size(); i++) {
            spawnnames = spawnnames.concat(pointlist.get(i).getName() + ", ");
        }
        return spawnnames;
    }
    ...
    private String addspawnpoint(String[] args, Location loc) {
        /*
         * "name:type" "name:type:period" "name:type:period:count"
        ...
        try {
            ...
            if (type == null)
                return "Invalid monster name " + args[2] + ".";
    ...
    public EntityType CTName(String name) {
    ...
    private String addspawnpoint(String[] args) {
        ...
        for (int i = 0; i < plugin.worldnames.size(); i++) {
            String worldname = plugin.worldnames.get(i);
            if (worldnamez.equals(worldname))
                break;
            if (i == plugin.worldnames.size() - 1)
                return "Invalid worldname or world not enabled for Monster Apocalypse.";
        }
    ...
    private void setconfig() {
        ...
        for (int n = 0; n < pointlist.size(); n++) {
            ...
            for (int i = 0; i < conflist.size(); i++) {
                if (conflist.get(i).equalsIgnoreCase(pointname)) {
                    continue;
                }
            }
    ...
    private void fillpointlist() {
        ...
        for (int n = 0; n < conflist.size(); n++) {
            pointname = conflist.get(n);
            if (pointname.equals("startlist"))
                continue;
            for (int i = 0; i < pointlist.size(); i++) {
                if (pointlist.get(i).getName().equalsIgnoreCase(pointname)) {
                    continue;
                }
            }
            try {
                ...
                System.out.println("Monster Apocalypse: Monster type " + plugin.conf.getString(pointname + ".type")
                                   + " not valid or CB Version too high, please use CB-1.0.1-R1.");
                continue;
            }
            try {
                ...
                System.out.println("Monster Apocalypse: World " + plugin.conf.getString(pointname + ".world") + " not found for spawnpoint "
                                   + plugin.conf.getString(pointname + ".name") + ", spawnpoint skipped.");
            }
        }
    }
    ...
    public void run() {
        for (int i = 0; i < pointlist.size(); i++) {
            pointlist.get(i).incrTime();
        }
        for (int i = 0; i < pointlist.size(); i++) {
            pointlist.get(i).tryspawn();
        }
    }
    ...
    private class spawnpoint {
        ...
        public void tryspawn() {
            for (int i = 0; i < plugin.worldnames.size(); i++) {
                String worldname = plugin.worldnames.get(i);
                if (pos.getWorld().getName().equals(worldname))
                    break;
                if (i == plugin.worldnames.size() - 1)
                    return;
            }
