...
public class DamageListener implements Listener {
    ...
    public void onEntityDamage(EntityDamageEvent event) {
        ...
        // TOTEMO: was CraftArror and CraftItem
        if (event.getEntity() instanceof Arrow
            || event.getEntity() instanceof Item || event.getEntity() instanceof Wolf
        ...
        if (!(plugin.changecombathp)) {
            ...
            if ((!(ent instanceof Player))
                ||
                (!plugin.checkaggro || (plugin.checkaggro && !((Player) ent).hasPermission("Monster Apocalypse.deaggro")))) {
                    ...
                    if (event.getCause() == DamageCause.ENTITY_ATTACK || event.getCause() == DamageCause.PROJECTILE) {
                        ...
                        if ((ent instanceof Player) && !(attacker instanceof Player)) {
                            ...
                            if (!(plugin.getMobName(attacker).equalsIgnoreCase("")) && !(attacker instanceof Wolf)
                                && !(plugin.getMobName(attacker).equalsIgnoreCase("Slime"))
                                && !(plugin.getMobName(attacker).equalsIgnoreCase("LavaSlime"))
                ...
                try {
                    ...
                    if (plugin.healths.getLastDamageTime(ent) > System.currentTimeMillis() - Math.min(500L, plugin.damageperiod)) {
                        ...
                        if (plugin.damageperiod < 500) {
                            ...
                            attacked.setVelocity(attacked.getVelocity()
                            .add(attacked.getLocation().toVector().subtract(attacker.getLocation().toVector()).normalize().multiply(knockback)));
        ...
        if (event.getCause() == DamageCause.ENTITY_ATTACK || event.getCause() == DamageCause.PROJECTILE) {
            ...
            if ((ent instanceof Player) && !(attacker instanceof Player)) {
                if (!plugin.checkaggro || (plugin.checkaggro && !((Player) ent).hasPermission("Monster Apocalypse.deaggro"))) {
                    if (plugin.potionhits) {
                        plugin.applyPotionEffects(ent, attacker);
                    }
                    if (plugin.changecombatdmg) {
